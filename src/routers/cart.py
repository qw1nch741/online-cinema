...
async def get_cart(db: AsyncSession = Depends(get_postgresql_db), current_user: UserModel = Depends(get_current_user)):
...
async def add_movie_to_the_cart(item: CartItemAdd,
                                db: AsyncSession = Depends(get_postgresql_db),
                                current_user: UserModel = Depends(get_current_user)):

    user_cart_result = await db.execute(select(CartModel).where(CartModel.user_id == current_user.id))
    ...
    result = await db.execute(select(CartItemModel).where(CartItemModel.cart_id == user_cart.id, CartItemModel.movie_id == item.movie_id))
    item_in_cart = result.scalar_one_or_none()
    if item_in_cart:
        raise HTTPException(status_code=404, detail="Movie is already purchased or a is duplicate")
